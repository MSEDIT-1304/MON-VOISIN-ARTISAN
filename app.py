...
def init_database():
    ...
    cursor.execute("""
        CREATE TABLE IF NOT EXISTS artisan_demandes_vues (
            artisan_id INTEGER NOT NULL,
            demande_id INTEGER NOT NULL,
            PRIMARY KEY (artisan_id, demande_id)
        )
    """)
        # ------------------------------------------------------
    # DEVIS
...
def inject_unread_messages():
    ...
    if user_type and user_id:
        ...
        if user_type == "artisan":
            ...
            if artisan:
                ...
                nouvelles_demandes = conn.execute(
                    """
                    SELECT demandes.*
                    FROM demandes
                    WHERE NOT EXISTS (
                    ...
                    AND NOT EXISTS (
                        ...
                        WHERE artisan_demandes_vues.artisan_id = ?
                        AND artisan_demandes_vues.demande_id = demandes.id
                    )
    ...
    return {
        ...
        "artisan_new_demandes": artisan_new_demandes
    }
...
def generer_pdf_devis(artisan, donnees):
    ...
    elements.append(
        Paragraph(
            f"<b>{texte_pdf(artisan['entreprise'])}</b><br/>"
            f"{texte_pdf(artisan['adresse'])}<br/>"
            f"{texte_pdf(artisan['code_postal'])} "
            f"{texte_pdf(artisan['ville'])}<br/>"
            f"Téléphone : {texte_pdf(artisan['telephone'])}<br/>"
            f"SIRET : {texte_pdf(artisan['siret'] or 'Non renseigné')}<br/>"
            f"TVA : {texte_pdf(artisan['tva'] or 'Non renseignée')}",
            texte_style
        )
    )
